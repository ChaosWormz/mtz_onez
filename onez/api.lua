onez = {}
function onez:register_mob(name, def)
...
onez.spawning_onez = {}
function onez:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, spawn_func)
	onez.spawning_onez[name] = true
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			if not onez.spawning_onez[name] then
				return
			end
			pos.y = pos.y+1
			...
			if minetest.setting_getbool("display_mob_spawn") then
				minetest.chat_send_all("[onez] Add "..name.." at "..minetest.pos_to_string(pos))
			end
...
function onez:register_arrow(name, def)
	minetest.register_entity(name, {
		physical = false,
		visual = def.visual,
		visual_size = def.visual_size,
